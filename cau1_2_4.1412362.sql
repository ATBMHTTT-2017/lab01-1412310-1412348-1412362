...
--4. Phan quyen cho giam doc
--tao view
CREATE OR REPLACE VIEW thongTinChiTietDuAn
AS
SELECT  DA.maDA AS maDA, DA.tenDA AS tenDA, DA.kinhPhi AS kinhPhi, PB.tenPhong AS tenPhongBan, CN.tenCN AS tenChiNhanh, NV.hoTen AS hoTenTruongDA, sum(CT.soTien) AS tongChiTieu
FROM DuAn DA, NhanVien NV, PhongBan PB, ChiNhanh CN, ChiTieu CT
...
GRANT SELECT ON system.thongTinChiTietDuAn TO giamDoc;

select* FROM system.thongTinChiTietDuAn;